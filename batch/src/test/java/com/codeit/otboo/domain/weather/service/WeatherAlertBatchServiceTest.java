...
class WeatherAlertBatchServiceTest {
    ...
    @Mock
    private AlertTargetQueryRepository alertTargetQueryRepository;
    ...
    class FindAlertTargetsByRegionTest {
        ...
        void findAlertTargetsByRegion_whenNoProfiles_returnsEmptyList() {
            // given
            when(alertTargetQueryRepository.findAllForWeatherAlert()).thenReturn(List.of());
        ...
        void findAlertTargetsByRegion_groupsProfilesByRegion() {
            // given
            AlertTarget alertTarget1 = new AlertTarget(UUID.randomUUID(), 60, 127);
            AlertTarget alertTarget2 = new AlertTarget(UUID.randomUUID(), 60, 127);
            AlertTarget alertTarget3 = new AlertTarget(UUID.randomUUID(), 55, 124);

            when(alertTargetQueryRepository.findAllForWeatherAlert())
                    .thenReturn(List.of(alertTarget1, alertTarget2, alertTarget3));
            ...
            assertThat(seoulTarget.userIds()).hasSize(2);
            assertThat(otherTarget.userIds()).hasSize(1);
        }
    }
    ...
    class BuildRegionAlertResultTest {
        ...
        void buildRegionAlertResult_whenTodayWeathersEmpty_returnsEmptyResult() {
            ...
            AlertTarget alertTarget = new AlertTarget(UUID.randomUUID(), 50, 120);
            RegionAlertTarget target = new RegionAlertTarget(50, 120, List.of(alertTarget.userId()));
        ...
        void buildRegionAlertResult_returnsTemperatureNotifications() {
            ...
            AlertTarget alertTarget1 = new AlertTarget(UUID.randomUUID(), 60, 127);
            AlertTarget alertTarget2 = new AlertTarget(UUID.randomUUID(), 60, 127);
            RegionAlertTarget target = new RegionAlertTarget(60, 127, List.of(alertTarget1.userId(), alertTarget2.userId()));
        ...
        void buildRegionAlertResult_whenNoAlerts_returnsEmptyResult() {
            ...
            AlertTarget alertTarget = new AlertTarget(UUID.randomUUID(), 55, 124);
            RegionAlertTarget target = new RegionAlertTarget(55, 124, List.of(alertTarget.userId()));
    ...
    class SaveAndPublishTest {
        ...
        void saveAndPublish_savesNotificationsAndPublishesEvents() {
            ...
            Assertions.assertThat(publishedEvent.notificationCommands()).hasSize(2);
            Assertions.assertThat(publishedEvent.notificationCommands())
                    .extracting(NotificationCreateCommand::title)
                    .containsOnly("어제와 기온 차가 커요");
            Assertions.assertThat(publishedEvent.notificationCommands())
                    .extracting(NotificationCreateCommand::content)
